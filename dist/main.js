...
/* harmony import */ var msgpack_lite__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(13);
/* harmony import */ var crypto_js_aes__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(46);
...
async function loadBlockchain() {
  let blocks = await localforage__WEBPACK_IMPORTED_MODULE_1___default().getItem('guzi-blocks');
  if (blocks !== null) {
    blocks = msgpack_lite__WEBPACK_IMPORTED_MODULE_2__.decode(blocks);
  }
...
function cypherAndSavePrivateKey(privateKey, pwd) {
  const cipherkey = crypto_js_aes__WEBPACK_IMPORTED_MODULE_3___default().encrypt(JSON.stringify(privateKey), pwd).toString();
...
function askPwdAndLoadPrivateKey(callback) {
  $("#pwdValidation").on("click", async () => {
    ...
    const bytes  = crypto_js_aes__WEBPACK_IMPORTED_MODULE_3___default().decrypt(cipherkey[0], pwd);
    if (bytes.sigBytes === 0) {
